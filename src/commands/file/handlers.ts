...
export async function renameHandler(treeItem: SnippetFileTreeItem) {
	const { renameSnippetFile } = await import('../../snippets/newSnippetFile.js');
	await renameSnippetFile(treeItem.filepath);
	refreshAll();
}
