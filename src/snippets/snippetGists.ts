...
async function importGist(context: vscode.ExtensionContext) {
    const identifier = await vscode.window.showInputBox({title: "Input a gist id, share url, or clone url"});
...
async function saveCodeSnippets(context: vscode.ExtensionContext, gist_id: string, saveDir: string): Promise<void> {
    const client = await getOctokitClient(context);
    ...
    let fileCount = 0;
    const includeAll = !vscode.workspace.getConfiguration("snippetstudio").get<boolean>("gists.onlySnippets");

    await Promise.all(Object.values(response.data.files).map(async file => {
        if (file && file.filename && file.content && (includeAll || file.filename.endsWith(".code-snippets"))) {
